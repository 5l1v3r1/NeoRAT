...
def keystroke(message):
  inject = validate_dict_key(message, 'inject')
  ...
  if inject:
    ...
    message['inject'] = read_file(f'{state["root"]}/{parent_folder}/{child_folder}/{script}').decode(state['settings']['encoding']).strip().lower().split('\r\n')
    del message['script']
  else:
